...
export default function YearlyRankingsChart({
    seasons
}: Readonly<{
        ...
        if (active && payload && payload.length) {
            return (
                <ChartTooltip>
                    ...
                    <p>{t("points")}: {formatNumber(payload[0].payload.points_for_year)}</p>
                </ChartTooltip>
            )
        };
    }

    const {domain, ticks} = calculateTickValues(data.map(d => d.rank_for_year));
